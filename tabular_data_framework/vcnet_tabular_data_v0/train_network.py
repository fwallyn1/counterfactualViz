...
import torch
from tabular_data_framework.load_data import Load_dataset_base,Load_dataset_carla
from torch import nn,optim
from torch.nn import functional as F 
from torch.utils.tensorboard import SummaryWriter
from itertools import product 
from tabular_data_framework.vcnet_tabular_data_v0.join_training_network import CVAE_join,Predictor
...
class Train_CVAE(Load_dataset_carla if set_carla() else Load_dataset_base) :
    def __init__(self, dataset_config_dict,model_config_dict,cat_arrays,cont_shape,loaders,dataset,ablation,condition,cuda_name,shared_layers=True,debug_enc=False):
        ...
        self.model = CVAE_join(dataset_config_dict,model_config_dict,self.cat_arrays,self.cont_shape,self.ablation,self.condition,shared_layers=self.shared_layers,debug_enc=self.debug_enc)
    ...
    def train_and_valid_cvae(self,tensorboard=False,seed=42) :
        tb = SummaryWriter(main_path + "/runs/logs/" + self.name + "/")
        ...
        torch.save(self.model.state_dict(),main_path + "save_models/" + self.name + "join-CVAE_" + str(self.ablation) + "_" + str(self.condition)+"shared_layers_" + str(self.shared_layers) + "used_carla" + str(self.used_carla) + "_seed_" + str(seed) +"_debug_enc" + str(self.debug_enc)) 
    ...
    def test(self,df,save_dataframe=False) : 
        ...
        if save_dataframe : 
            ...
            df["Sparsity"].loc[str(self.name)] = str(round(batch_mean_sparsity,round_number)) + "+/-" + str(round(batch_std_sparsity,round_number))
        
         
        
    # Load weights for a trained model 
    def load_weights(self,name,seed=42) : 
        self.model.load_state_dict(torch.load(main_path + "save_models/" + name+ "join-CVAE_" + str(self.ablation) + "_" + str(self.condition) +"shared_layers_" + str(self.shared_layers) + 		"used_carla" + str(self.used_carla) + "_seed_" + str(seed) +"_debug_enc" + str(self.debug_enc)
    ...
    def round_counterfactuals(self,result,eps,X) : 
        ...
        percentage = torch.sum(predicted_counterfactuals_round_classes== 1 - label_examples) /predicted_counterfactuals_round_classes.shape[0] 
    ...
    def objective(self,trial) :  
        ...
        self.lambda_1,self.lambda_2,self.lambda_3 =  trial.suggest_float("lambda_1", 0.001,1,step=0.001) , trial.suggest_float("lambda_2", 0.001,1,step=0.001) , trial.suggest_float("lambda_3", 0.1,1,step=0.1)
